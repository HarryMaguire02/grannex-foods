...
const steps = items.filter((item): item is Step => item.type === 'step');
    ...
    <section className="bg-white py-8 lg:py-12">
      <div className="max-w-content mx-auto px-6 sm:px-8 lg:px-12">
          ...
          {items.map((item) =>
            item.type === 'step' ? (
              <AnimateIn
                key={item.number}
                from="bottom"
                delay={steps.findIndex((s) => s.number === item.number) * 0.15}
        ...
        <div className="flex flex-col gap-6 lg:hidden">
          {steps.map((step, i) => (
            <AnimateIn key={step.number} from="bottom" delay={i * 0.1}>
